...
GLint TextureFromFile(const char* path, std::string directory) {
    ...
    GLenum fmt;
    if (ch == 1)      fmt = GL_RED;  // Para imágenes en escala de grises
    else if (ch == 3) fmt = GL_RGB;  // Para JPGs normales
    else if (ch == 4) fmt = GL_RGBA; // Para PNGs con transparencia
    else              fmt = GL_RGB;

    glBindTexture(GL_TEXTURE_2D, id);

    // Alerta, si se usa GL_RED, se necesita esta validacion para que se vea blanco y no rojo:
    if (ch == 1) {
...
static GLuint TextureFromEmbedded(const aiTexture* tex) {
    ...
    if (tex->mHeight == 0) {
        ...
        else if (ch == 2) fmt = GL_RG;
        else fmt = GL_RED;

        glBindTexture(GL_TEXTURE_2D, id);

        // Condicionales para corregir tonos verdes y rojos ---
        if (ch == 1) {
            GLint swizzleMask[] = { GL_RED, GL_RED, GL_RED, GL_ONE };
            glTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_RGBA, swizzleMask);
        }
        else if (ch == 2) {
            GLint swizzleMask[] = { GL_RED, GL_RED, GL_RED, GL_GREEN }; // R,G usado como escala de grises + alpha
            glTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_RGBA, swizzleMask);
        }
